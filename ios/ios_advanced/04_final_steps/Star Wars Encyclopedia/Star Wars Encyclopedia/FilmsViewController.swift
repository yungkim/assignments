...
class FilmsViewController: UITableViewController, FilmDetailsViewControllerDelegate {
                ...
                if let jsonResult = try NSJSONSerialization.JSONObjectWithData(data!, options: NSJSONReadingOptions.MutableContainers) as? NSDictionary {
                    if let results = jsonResult["results"] {
                        for i in 0..<results.count {
                            ...
                            let url = results[i].valueForKey("url")!
                            self.url.append(url as! String)
    ...
    override func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        let sendUrl = url[indexPath.row]
        performSegueWithIdentifier("toFilmDetails", sender: sendUrl)
    }
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        let destination = segue.destinationViewController as! FilmDetailsViewController
